...
**Then match those habits.** Do not replace a casual word with a formal one. Do not remove a quirk the writer actually uses. Do not sand the sample's rhythm down to the even surface the rules would otherwise produce. The point of the sample is that it is uneven and specific, and the draft should be too.
...
This matters because machine text is measurably dense and noun-heavy and does not vary with genre (Reinhart et al.). Instruction-tuned models hold one register from the first line to the last. The venue corpus is what supplies the variation the model does not produce on its own.
