# Domain — release notes and announcements

Covers changelogs, GitHub Releases, version announcements, and short launch posts. Run with `SKILL.md`. Add `references/ai-tells.md` for anything longer than a changelog.
...
2. Every claim carries its artifact: issue and PR numbers, commit ranges, exact version strings, real benchmark numbers with conditions. No artifact means no claim.
...
5. Humor and voice are fine if the repo's history has them. Never inject them fresh into a repo that does not.
