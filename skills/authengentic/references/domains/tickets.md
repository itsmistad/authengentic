...
Covers issue tickets, tasks, work orders, bug reports you file (not replies — that's `dev-replies.md`). Run with `SKILL.md` (short-answer weighting).

## Human baseline

Imperative, minimal, complete-enough. The assignee can start without asking a question, and knows when the work is done. A tracker's field template is a container, not a tell.
...
| Round scope words: "refactor the module", "clean up" | The concrete boundary: which files and functions in, which explicitly out |

## Rules

1. **Title = outcome**, not activity ("Retry queue drops jobs on redeploy" not "Investigate queue issue").
2. Bug tickets: exact repro (versions, commands, input), expected versus actual with real output pasted, frequency. If you cannot reproduce it, say what you tried.
